...
#nullable  enable
    internal class DbFactory : IDisposable
    {
        ...
        {
            DbCommand command = Shared.Type switch
            {
        ...
        {
            fieldName = fieldName.Replace(Shared.Operator, "");
            string name = $"{Shared.Operator}{fieldName}";
            return Shared.Type switch
            {
        ...
        internal DbParameter[] CreateParameters(IList<Option> values)
        {
            var parameters = new DbParameter[values.Count];
            for (var index = 0; index < values.Count; index++)
                parameters[index] = CreateParameter($"{Shared.Operator}{values[index].FieldName}", values[index].Value);
            return parameters;
        }

    ...
    internal static class Mapper
    {
        ...
        internal static T? Map<T>(IDataRecord idr) where T : class, new()
        {
            ...
            for (int index = 0; index < mappings.Count; index++)
            {
                ...
                string name = col.Name.ToLower() == pi.Name.ToLower() ? pi.Name : col.Name;
                ...
                if (nullable != null)
                {
                    pi.SetValue(result, Convert.ChangeType(idr[name], nullable), null);
                    continue; /* Step to the next property */
                }

                if (pi.PropertyType.IsEnum)
                {
                    pi.SetValue(result, Enum.ToObject(pi.PropertyType, idr[name]));
                    continue;
                }

                pi.SetValue(result, Convert.ChangeType(idr[name], pi.PropertyType));
            }

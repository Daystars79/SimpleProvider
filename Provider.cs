...
namespace SimpleProvider
{
    ...
    public class Provider : IDisposable
    {
        ...
        /// <summary>
        /// Return the DatabaseType of the Current Instance
        /// </summary>
        public ProviderType Type => Shared.Type;
        ...
        {
            CommandSet cs = CommandSets.CreateSelect(record);
        ...
        {
            CommandSet cs = CommandSets.CreateSelect<T>(args);
        ...
        {
            CommandSet cs = CommandSets.CreateTop<T>(count, args);
        ...
        public IList<object>? GetTopRecords(int count, Type type, params Option[] args)
        {
            ...
            CommandSet cs = CommandSets.CreateTop(count, type, args);
            using DbCommand dc = _factory.CreateCommand(cs, _timeout);
        ...
        public bool Insert(object record)
        {
            ...
            CommandSet cs = CommandSets.CreateInsert(record);
        ...
        public bool Update(object record, params Option[] args)
        {
            ...
            cs = CommandSets.CreateUpdate(record, dbRef, args);
        ...
        public bool Delete(object record)
        {
            if (record == null) throw new ArgumentNullException(nameof(record));
            if (!Exists(record)) return false;
            CommandSet cs = CommandSets.CreateDelete(record);
            using DbCommand command = _factory.CreateCommand(cs);
            return command.ExecuteNonQuery() > 0;
        }
        ...
        {
            if (record == null) throw new ArgumentNullException(nameof(record));
            CommandSet cs = CommandSets.CreateExists(record);
